    ...
    , proxyModel(new QSortFilterProxyModel(this)) // Инициализация прокси-модели для сортировки и фильтрации
{
    ...
    ui->comboBoxSearchField->addItem("Все поля");
    ui->comboBoxSearchField->addItem("Имя");
    ui->comboBoxSearchField->addItem("Отчество");
    ui->comboBoxSearchField->addItem("Фамилия");
    ui->comboBoxSearchField->addItem("Дата");
    ui->comboBoxSearchField->addItem("Электронная почта");
    ui->comboBoxSearchField->addItem("Телефон");
...
void MainWindow::SetupModel()
{
    ...
    QStringList headers = { "Имя", "Отчество", "Фамилия", "Дата", "Электронная почта", "Телефон" };     //Заголовки таблицы
