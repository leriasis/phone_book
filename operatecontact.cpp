...
// Добавление контакта
void OperateContact::AddContact(const Contact &contact)
{
...
void OperateContact::RemoveContact(size_t index)
{
...
void OperateContact::UpdateContact(size_t index, const Contact &contact)
{
    if (index >= Contacts.size()) {
        throw std::out_of_range("Index out of range");
    }
...
std::vector<Contact> OperateContact::SearchContacts(const std::function<bool(const Contact &)> &criteria) const
{
...
Contact OperateContact::GetContact(size_t index) const
{
    if (index >= Contacts.size()) {
        throw std::out_of_range("Index out of range");
    }
...
const std::vector<Contact> &OperateContact::GetAllContacts() const
{
    return Contacts;
}

// Сортировка контактов по критерию
void OperateContact::SortContacts(const std::function<bool(const Contact &, const Contact &)> &comparator)
{
    std::sort(Contacts.begin(), Contacts.end(), comparator);
}

// Загрузка контактов из файла
void OperateContact::LoadFromFile(const std::string &filename)
{
...
void OperateContact::SaveToFile(const std::string &filename) const
{
...
void OperateContact::LoadFromDB(const QString &connectionName) {
...
void OperateContact::SaveToDB(const QString &connectionName) {
    // Проверяем, существует ли соединение с данным именем
    if (!QSqlDatabase::contains(connectionName)) {
        db = QSqlDatabase::addDatabase("QSQLITE", connectionName);
        db.setDatabaseName("contacts.db");
